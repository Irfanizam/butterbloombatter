  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-brand-lg border border-brand-border-soft bg-white shadow-brand-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-brand-lg">
      ...
      >
        ...
        />
        {product.isFeatured && (
          <span className="absolute left-3 top-3 z-10 rounded-full bg-brand-accent-light px-2.5 py-0.5 text-xs font-bold text-brand-gold shadow-brand-sm">
            ⭐ Featured
          </span>
        )}
        {product.pieces != null && (
          <span className="absolute bottom-2 left-3 z-10 rounded-full bg-white/90 px-2.5 py-0.5 text-xs font-bold text-brand-dark shadow-brand-sm">
            🍪 {product.pieces} pcs
          </span>
        )}
      </button>
